...
function handlevalue() {
	try {
		window.onload = async () => {
			...
			const mainelementsofstories =
				document.querySelector(`#f22-storiesplugin1`);
			if (mainelementsofstories) {
				let el = document.createElement("f22-stories");
				//@ts-ignore
				let result = window.Shopify?.shop?.split(".")[0] || "dw-speakers";
				// let  result = window.Shopify?.shop?.split(".")[0];
				// console.log(result);
				console.log(window.location);
				el.setAttribute(
					"dataUrl",
					`https://s3.f22labs.cloud/shopclips/${result}.json`
				);
