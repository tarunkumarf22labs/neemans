...
const ProductCard = ({ productname, stopProgress, isOpen, setIsOpen, triggers }: Props) => {
    ...
    const handleOpenProductDetails = () => {
        ...
        triggers.dotclickedtoupdate(triggers.productid || triggers.data?.childstories[triggers.actualTime]?.dots?.[0]?.productname, triggers.data?.childstories[triggers.actualTime], triggers.data)
        setIsOpen((prev) => !prev);
        stopProgress();
        if (isOpen) {
            startProgress();
    ...
    return (
        <div className="product-card">
            <div className="product-card-content">
                ...
                <div className="product-card-img" onClick={() => handleOpenProductDetails()}>
                    <img src={product?.images[0].image} alt={product?.title} />
                </div>
                <div className="product-card-info" onClick={() => handleOpenProductDetails()}>
                    <span className="product-card-info-title">{product?.title}</span>
                    <span className="product-card-info-price">£{product?.variants[0].price}</span>
                </div>
                <div className={`product-card-variants ${isVariantSelectorOpen ? "product-variant-open" : ""}`} >
                    {product?.variants?.map((variant, index) => (
                        <div className={`product-card-variant ${selectedVariantIndex == index ? "product-card-variant-active" : ""}`} onClick={() => handleVariantSelection(variant.id, index)}>{variant?.title}</div>
                    ))}
                </div>
            </div>
            {isVariantSelectorOpen ?
                (<a className="add-to-cart-product-card" href={`https://paperlondon.com/cart/add?id=${variant}&quantity=1&size=6`}> Add to Cart</a>)
                : (<button className="add-to-cart-product-card" onClick={() => {
            ...
            <div id="powered_by_product_card">
                <span>Powered By <svg width="18" height="18" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
