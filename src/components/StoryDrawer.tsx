...
function StoryDrawer({
    ...
    async function fetchData() {
      try {
        const data = await fetch(
          `https://www.femmessanspeur.com/products/${productname}.xml`,
  ...
  const handleAddToCart = () => {
    ...
    const url = 'https://www.femmessanspeur.com/cart/add';
 ...
 {textforCart}
            ...
            <a href={`https://www.femmessanspeur.com/cart/${variant.id}:1?checkout`} className="atc_button" onClick={()=>{getClickdata("BUYNOW")}}>
              BUY NOW
            </a>
          </div>
        </>
