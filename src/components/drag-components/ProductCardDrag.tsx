...
const ProductCard = ({ productname, setIsOpen, setproductName }: any) => {
    ...
    async function fetchData() {
      try {
        const data = await fetch(
          `https://www.9shineslabel.com/products/${productname.name}.xml`,
          { redirect: "follow" }
        );
  ...
  const handleAddToCart = () => {
    ...
    fetch(url, {
      ...
      .then((response) => {
        if (response.ok) {
          window.location.href = "https://www.9shineslabel.com/cart";
        } else {
  ...
  return (
    <div className="drag-product-card">
      <div className="drag-product-card-content">
        <div
          className="drag-product-card-img"
        ...
        <div
          className="drag-product-card-info"
          style={{ cursor: "pointer" }}
          onClick={() => handleOpenProductDetails()}
        >
          <span className="drag-product-card-info-title">{product?.title}</span>
          <span className="drag-product-card-info-price">
            £{product?.variants?.[0]?.price}
          </span>
        </div>
        <div
          className={`drag-product-card-variants ${
            isVariantSelectorOpen ? "drag-product-variant-open" : ""
          }`}
        >
          {product?.variants?.map((variant, index) => (
            <div
              className={`drag-product-card-variant ${
                selectedVariantIndex == index
                  ? "drag-product-card-variant-active"
                  : ""
              }`}
      ...
      {isVariantSelectorOpen ? (
        <button className="drag-add-to-cart-product-card" onClick={handleAddToCart}>
          Add to Cart
        </button>
      ) : (
        <button
          className="drag-add-to-cart-product-card"
          onClick={() => {
      ...
      <div id="drag-powered_by_product_card">
        <span>
          Powered By{" "}
          <svg
            width="18"
