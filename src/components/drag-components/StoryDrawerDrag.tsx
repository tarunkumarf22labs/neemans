...
import CustomSliderDrag from "./CustomSliderDrag";
import { Variant, shopify } from "../../types";
import { memo } from "preact/compat";
import PoweredBy from "../PoweredBy";
import Loader from "../Loader";
import { getClickdata } from "../../hook/firebase";
import './StoryDrawerDrag.css'
function StoryDrawerDrag({
  setIsOpen,
  isSizeOpen,
  setIsSizeOpen,
  productname,
  ...
  return (
    <div
      className="drag-plugin-inner_container"
      onClick={(e) => {
        e.stopPropagation();
      }}
    >
      {product ? (
        <>
          ...
          <div
            className="drag-size_container">
            <button  disabled={textforCart === "Add to cart"  ? false : true   }  onClick={() => {handleAddToCart(); getClickdata("ADD_TO_CART")}} className="drag-atc_button" style={{ cursor: "pointer" }} >
 {textforCart}
             </button>
            <a href={`https://www.9shineslabel.com/cart/${variant.id}:1?checkout`} className="drag-atc_button" onClick={()=>{getClickdata("BUYNOW")}}>
              BUY NOW
            </a>
          </div>
        </>
...
export const MemoizedStoryDrawerDrag = memo(StoryDrawerDrag);
